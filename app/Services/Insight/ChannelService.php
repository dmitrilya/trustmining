...
class ChannelService
{
    ...
    public function store(User $user, string $name, string $slug, string $briefDescription, string $description, UploadedFile $logo, ?UploadedFile $banner): Channel
    {
        ...
        if ($banner) {
            $time = time();
            $channel->banner = $this->saveFile($banner, 'insight/' . $slug, 'banner', $channel->id, $time, [960, 360], null, 90);
            $this->saveFile($banner, 'insight/' . $slug, 'banner', $channel->id, $time, [416, 156], null, 90);
        }
        $channel->save();
