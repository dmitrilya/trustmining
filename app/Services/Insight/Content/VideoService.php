...
class VideoService extends ContentService
{
    ...
    public function store(Channel $channel, array $data): ?ContentModel
    {
        ...
        $video->preview = $this->saveFile($data['preview'], 'insight/' . $channel->slug, 'video_preview', $video->id, 'public', false, true, 100);
        $video->save();
    ...
    public function update(Channel $channel, ContentModel $video, array $data): ?ContentModel
    {
        ...
        if ($data['preview']) $changings['preview'] = $this->saveFile($data['preview'], 'insight/' . $channel->slug, 'video_preview', $video->id, 'public', false, true, 100);
