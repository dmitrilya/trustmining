...
class AsicRatingController extends Controller
{
    ...
    {
        $result = Cache::remember("rating_asics_$type" . ($filter ? "_$filter" : ''), now()->addHours(6), function () use ($type, $filter) {
            ...
            elseif ($type == 'profit') $models = $models->map(function ($model) use ($filterType, $filterValue, $rub) {
