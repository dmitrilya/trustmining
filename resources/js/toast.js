...
const pushToast = function (type, title, pretext, text, alert) {
    ...
    const alertIcons = {
        ...
        error: `<div class="inline-flex items-center justify-center flex-shrink-0 w-8 h-8 text-red-500 bg-red-100 rounded-full dark:bg-red-700 dark:text-red-100">
        <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
    ...
    toast.className = "w-full p-4 bg-white/40 dark:bg-slate-900/40 border border-slate-300 dark:border-slate-700 backdrop-blur-2xl rounded-lg shadow-lg shadow-logo-color border border-slate-300 dark:border-slate-700 transition duration-300";
    toast.role = 'alert';
    ...
    <span class="text-xs text-slate-500 dark:text-100">${title}</span>
    <button type="button" onclick="this.parentElement.parentElement.style.opacity = 0;setTimeout(() => { this.parentElement.parentElement.remove() }, 300)"
        class="block ml-auto -mx-1.5 -my-1.5 bg-white/40 dark:bg-slate-900/40 border border-slate-300 dark:border-slate-700 justify-center items-center flex-shrink-0 text-slate-500 hover:text-slate-900 dark:hover:text-slate-200 rounded-lg p-1.5 hover:bg-slate-100 dark:hover:border-slate-800 inline-flex h-8 w-8 dark:text-slate-400" aria-label="Close">
        <span class="sr-only">Close</span>
        <svg class="w-3 h-3" aria-hidden="true" fill="none" viewBox="0 0 14 14">
...
<div class="font-semibold text-slate-950 mb-1">${pretext}</div>
