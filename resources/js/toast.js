...
const pushToast = function (type, title, pretext, text, alert) {
    ...
    toast.className = "w-full p-4 bg-white/40 dark:bg-slate-900/40 border border-slate-300 dark:border-slate-700 backdrop-blur-xl rounded-lg shadow-lg shadow-logo-color transition duration-300";
    toast.role = 'alert';
    toast.innerHTML = type == 'alert' ? `<div class="flex items-center justify-between">
    ${alertIcons[alert]}
    <div class="ml-3 text-sm text-slate-600 dark:text-slate-400">${text}</div>
    ...
    <span class="text-xs text-slate-600 dark:text-slate-400">${title}</span>
    <button type="button" onclick="this.parentElement.parentElement.style.opacity = 0;setTimeout(() => { this.parentElement.parentElement.remove() }, 300)"
        class="block ml-2 bg-white/40 dark:bg-slate-900/40 border border-slate-300 dark:border-slate-700 justify-center items-center flex-shrink-0 text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 rounded-lg p-1.5 hover:bg-slate-100 dark:hover:border-slate-800 inline-flex h-8 w-8 dark:text-slate-400" aria-label="Close">
        <span class="sr-only">Close</span>
        <svg class="w-3 h-3" aria-hidden="true" fill="none" viewBox="0 0 14 14">
...
<div class="font-semibold text-slate-800 mb-1">${pretext}</div>
