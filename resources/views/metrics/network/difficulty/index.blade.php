<x-metrics-layout title="Сложность сети {{ $coin->name }}" :header="__('Network difficulty') . ' ' . $coin->name" active="network_difficulty"
    ...
    <div x-data="{ period: '1y', items: [] }" x-init="axios.get('{{ route('metrics.network.get_difficulty', ['coin' => $coin->name]) }}').then(r => {
        ...
        <div class="bg-white/60 dark:bg-zinc-900/60 border border-gray-300 dark:border-zinc-700 overflow-hidden shadow-sm shadow-logo-color rounded-lg p-2 sm:p-4 md:p-6 graph-container">
            <div class="flex justify-between md:justify-end items-start mb-3 xs:mb-4 lg:mb-6">
                <div class="bg-gray-100 dark:bg-zinc-900 size-7 sm:size-8 rounded-md shadow-sm shadow-logo-color cursor-pointer border dark:border-zinc-700 flex justify-center items-center md:hidden"
                    @click="show = !show">
            ...
            <div class="text-center my-4 xs:my-5 sm::my-6 md:my-7 lg:my-9">
                ...
                @if ($prediction)
                    <div class="text-xxs sm:text-xs lg:text-sm text-gray-700 dark:text-gray-200 mb-2 sm:mb-3 lg:mb-4">
                        {{ __('Blocks before recalculation') }}: <span
                            class="font-bold">{{ $difficulty->need_blocks }}</span> ({{ $needBlocksTime }})
                    </div>

                    <div class="text-sm sm:text-base lg:text-lg text-gray-900 dark:text-gray-100 font-bold">
        ...
        <div class="bg-white/60 dark:bg-zinc-900/60 border border-gray-300 dark:border-zinc-700 overflow-hidden shadow-sm shadow-logo-color rounded-lg mt-4 sm:mt-6 p-2 sm:p-4 md:p-6">
