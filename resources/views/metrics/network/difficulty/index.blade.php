    ...
    <div x-data="{ period: '3m', items: [] }" x-init="axios.get('{{ route('metrics.network.get_difficulty', ['coin' => $coin->name]) }}').then(r => {
                ...
                <x-select name="coin_id" :key="$coin->id" :items="\App\Models\Coin::has('networkDifficulties')
                    ...
                    ->map(
                        fn($coin) => [
                            ...
                            'href' => route('metrics.network.difficulty', ['coin' => $coin->name]),
                        ],
                    )
        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-lg mt-4 sm:mt-6 p-2 sm:p-4 md:p-6">
                <div class="grid grid-cols-6 gap-2 sm:gap-3 mb-2 sm:mb-3">
                    <div class="col-span-2 font-bold text-xs sm:text-sm lg:text-base text-gray-500 dark:text-gray-400">{{ __('Date') }}</div>
                ...
                <template x-for="(item, i) in items.slice(0, 90)" key="item.date">
                    <div class="grid grid-cols-6 gap-2 sm:gap-3 mb-1 sm:mb-2">
                        ...
                        <div class="col-span-3 text-xxs xs:text-xs sm:text-base lg:text-lg text-gray-700 dark:text-gray-300" x-text="item.value"></div>
