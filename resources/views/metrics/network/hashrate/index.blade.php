<x-metrics-layout title="Хэшрейт сети {{ $coin->name }}" :header="__('Network hashrate') . ' ' . $coin->name" active="network_hashrate"
    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-lg p-4 md:p-6" x-data="{ period: '3m' }"
        x-init="axios.get('{{ route('metrics.network.get_hashrate', ['coin' => $coin->name]) }}').then(r => window.buildGraph(r.data.hashrates, period))">
        <div class="flex justify-end space-x-2 xs:space-x-3 sm:space-x-4 mb-3 xs:mb-4 lg:mb-6">
            ...
            <x-select name="coin_id" :key="$coin->id" :items="\App\Models\Coin::has('networkHashrates')
                ->get()
                ->map(fn($coin) => ['key' => $coin->id, 'value' => $coin->abbreviation, 'href' => route('metrics.network.hashrate', ['coin' => $coin->name])])
