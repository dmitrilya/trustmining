<x-app-layout title="Каталог газопоршневых электростанций"
    ...
    <div class="max-w-7xl mx-auto px-2 py-4 sm:p-6 lg:p-8">
        <x-breadcrumbs>
            <x-breadcrumb position="1" :name="__('Gas gensets')" />
        </x-breadcrumbs>
            ...
            <div class="flex flex-wrap sm:grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
                @foreach ($brands as $brand)
                    <a x-data="{ name: '{{ $brand->name }}'.toLowerCase() }" x-show="name.includes(search.toLowerCase())"
                        href="{{ route('database.gas-gensets.brand', ['gpuBrand' => $brand->slug]) }}"
        ...
        <div class="mt-4 sm:mt-6 bg-white/40 dark:bg-slate-900/40 border border-slate-300 dark:border-slate-700 overflow-hidden shadow-sm shadow-logo-color rounded-lg p-4 md:p-6"
            x-data="{
                models: {{ $brands->pluck('gpuModels')->flatten() }},
                search: null,
                sortAsc: false,
                sortCol: null,
                sort(col, asc = true) {
