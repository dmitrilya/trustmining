<x-modal name="login" maxWidth="sm" rounded="rounded-xl">
    <div class="p-6 lg:p-8">
        ...
        <form x-data="{
            ...
            cleanRedirectUrl: '{{ url()->current() }}'
        }"
            @open-login-modal.window="
                if ($event.detail.redirect) {
                    cleanRedirectUrl = $event.detail.redirect;
                    action = '{{ route('login') }}?redirect=' + encodeURIComponent($event.detail.redirect);
                } else {
                    cleanRedirectUrl = '{{ url()->current() }}';
                    action = '{{ route('login', ['redirect' => url()->current()]) }}';
                }
            ...
            <div class="flex items-center justify-end mt-4">
                <button type="button" @click="show = false; $dispatch('open-register-modal', { redirect: cleanRedirectUrl })"
