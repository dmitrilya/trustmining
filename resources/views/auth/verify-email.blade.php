<x-guest-layout title="Подтверждение email - завершение регистрации в TrustMining"
    ...
    <div class="mt-4">
        ...
        <form class="mt-3 sm:mt-4" method="POST" action="{{ route('logout') }}">
            ...
            <button type="submit"
                class="block ml-auto underline text-sm text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-slate-800">
                {{ __('Logout') }}
            </button>
        </form>
    </div>
