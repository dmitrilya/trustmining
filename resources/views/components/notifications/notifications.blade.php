...
<div class="w-5 h-5" x-data="{ open: false, opened: false }">
    <button @click.stop="if(!opened) { opened = true; $nextTick(() => open = true) } else { open = !open }; checkNotifications()"
        aria-label="{{ __('Notifications') }}" type="button"
    ...
    <template x-if="opened">
        <div x-show="open" @click.away="open = false"
            ...
            <div class="w-full flex-auto bg-slate-100/95 dark:bg-slate-900/95 text-sm leading-6">
                <div class="block px-4 py-2 text-center text-slate-800 rounded-t-lg bg-slate-50 dark:bg-slate-900 dark:text-slate-200">
                    {{ __('Notifications') }}
                </div>
                ...
                <a href="{{ route('notifications') }}"
                    ...
                    <div class="inline-flex items-center ">
                        <svg class="w-4 h-4 mr-2 text-slate-500" aria-hidden="true" fill="currentColor" viewBox="0 0 20 14">
                            <path
