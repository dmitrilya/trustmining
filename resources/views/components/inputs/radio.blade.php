...
<div class="{{ $class }}">
    <label class="flex items-center cursor-pointer group">
        <input @if ($handleChange) @change="{{ $handleChange }}($el.value)" @endif type="radio"
            {{ $attributes->merge(['class' => 'sr-only peer']) }}>

        <div
            class="{{ $sm ? 'mr-1 sm:mr-2 w-3 h-3 sm:w-4 sm:h-4' : 'mr-2 w-4 h-4' }} flex items-center justify-center shrink-0 bg-slate-100 dark:bg-slate-900 peer-checked:bg-indigo-600 border border-slate-300 dark:border-slate-700 rounded-full transition duration-100 text-transparent peer-checked:text-white peer-disabled:opacity-50 peer-disabled:cursor-not-allowed">
