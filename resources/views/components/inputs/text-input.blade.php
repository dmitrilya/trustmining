...
@if ($type === 'password')
    <div x-data="{ show: false }" class="relative mt-1">
        <input :type="show ? 'text' : 'password'"
            {{ $attributes->merge(['class' => 'py-1.5 px-3 block w-full rounded-lg shadow shadow-logo-color text-slate-800 bg-white dark:bg-slate-950 dark:text-slate-200 border-0 ring-1 ring-inset ring-slate-300 dark:ring-slate-700 focus:ring-indigo-500 dark:focus:ring-indigo-500 focus:outline-none disabled:cursor-not-allowed disabled:opacity-25']) }}>
...
@else
    <input type="{{ $type }}"
        {{ $attributes->merge(['class' => 'py-1.5 px-3 block mt-1 w-full rounded-lg shadow shadow-logo-color text-slate-800 bg-white dark:bg-slate-950 dark:text-slate-200 border-0 ring-1 ring-inset ring-slate-300 dark:ring-slate-700 focus:ring-indigo-500 dark:focus:ring-indigo-500 focus:outline-none disabled:cursor-not-allowed disabled:opacity-25']) }}>
@endif
