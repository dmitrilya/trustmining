...
<div x-data="{ open: false, items: {{ $items }}, itemKey: '{{ $key ? $key : $items->first()['key'] }}' }">
    ...
    <div class="relative flex min-w-max{{ isset($label) ? ' mt-1' : '' }}">
        ...
        <ul x-show="open" @click.away="open = false" style="display: none"
            class="absolute top-full z-10 mt-1 max-h-56 w-full overflow-auto rounded-md bg-white/60 dark:bg-zinc-900/60 backdrop-blur-2xl py-1 text-base shadow-lg shadow-logo-color ring-1 ring-black dark:ring-zinc-700 ring-opacity-5 focus:outline-none sm:text-sm">
