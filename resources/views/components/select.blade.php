@props(['label' => null, 'name', 'items', 'key' => null, 'icon' => null, 'handleChange' => null])
...
<div x-data="{ open: false, items: {{ $items }}, itemKey: {{ $key ? $key : $items->first()['key'] }} }">
    ...
    <input type="hidden" name="{{ $name }}" :value="itemKey"
        @if ($handleChange) @change="{{ $handleChange }}(itemKey)" @endif>

    <div class="relative min-w-max{{ isset($label) ? ' mt-1' : '' }}">
        <button type="button" @click="open = ! open"
            ...
            <span class="flex items-center">
                @if ($icon)
                    @switch($icon['type'])
                        @case('value')
                            <img class="mr-2 xs:mr-3 w-3 h-3 xs:w-4 xs:h-4 sm:w-5 sm:h-5"
                        ...
                        @case('path')
                            <img class="mr-2 xs:mr-3 w-3 h-3 xs:w-4 xs:h-4 sm:w-5 sm:h-5" :src="items[itemKey].icon"
                                :alt="items[itemKey].value">
                        @break
                    @endswitch
                @endif

                <span class="block truncate text-xs sm:text-sm" x-text="items[itemKey].value"></span>
            </span>
        ...
        <ul x-show="open" @click.away="open = false" style="display: none"
            ...
            @foreach ($items as $item)
                <li @if (isset($item['href'])) @click="window.location.href = '{{ $item['href'] }}'" @else @click="itemKey = {{ $item['key'] }};open = false" @endif
                    ...
                    <div class="flex items-center w-full">
                        @if ($icon)
                            @switch($icon['type'])
                                ...
                                @case('path')
                                    <img class="mr-2 xs:mr-3 w-3 h-3 xs:w-4 xs:h-4 sm:w-5 sm:h-5" src="{{ $item->icon }}"
                                        alt="{{ $item['value'] }}">
                                @break
                            @endswitch
                        @endif

                        <span class="block truncate text-xs sm:text-sm">{{ $item['value'] }}</span>
                    </div>
