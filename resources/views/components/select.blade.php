...
<div x-data="{ open: false, items: {{ $items }}, itemKey: '{{ $key ? $key : $items->first()['key'] }}' }">
    ...
    <div class="relative flex min-w-max{{ isset($label) ? ' mt-1' : '' }}">
        <button type="button" @click="open = ! open"
            class="relative w-full bg-white/60 dark:bg-zinc-900/60 border border-gray-300 dark:border-zinc-700 py-1.5 pl-3 {{ $size == 'sm' ? 'pr-5 sm:py-1.5 sm:pl-3 sm:pr-10 rounded-sm sm:rounded-md' : 'pr-10 rounded-md' }} text-left text-gray-950 dark:text-zinc-200 shadow-sm shadow-logo-color ring-1 ring-inset ring-gray-300 dark:ring-zinc-700 focus:outline-none focus:ring-indigo-500 dark:focus:ring-indigo-600">
            ...
            <span class="pointer-events-none absolute inset-y-0 right-0 ml-3 flex items-center pr-2">
                <svg class="{{ $size == 'sm' ? 'h-2.5 w-2.5' : 'h-4 w-4' }} sm:h-5 sm:w-5 text-gray-500 dark:text-gray-400"
        ...
        <ul x-show="open" @click.away="open = false" style="display: none"
            class="absolute top-full z-50 mt-1 max-h-56 w-full overflow-auto rounded-md bg-white/60 dark:bg-zinc-900/60 border border-gray-300 dark:border-zinc-700 backdrop-blur-2xl py-1 text-base shadow-lg shadow-logo-color ring-1 ring-black dark:ring-zinc-700 ring-opacity-5 focus:outline-none sm:text-sm">
            @foreach ($items as $item)
                <li @if (isset($item['href'])) @click="window.location.href = '{{ $item['href'] }}'"
                    ...
                    class="flex items-center justify-between cursor-pointer select-none py-2 px-3 text-gray-950 dark:text-gray-100 hover:bg-indigo-600 hover:text-white">
