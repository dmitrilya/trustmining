...
<div itemprop="mainEntity" itemscope itemtype="https://schema.org"
    class="border rounded-xl overflow-hidden shadow-sm bg-white/40 dark:bg-slate-900/40 border-l-4 border-l-indigo-600 dark:border-l-indigo-600 transition-all duration-300"
    ...
    <button type="button" @click="active !== {{ $i }} ? active = {{ $i }} : active = null"
        ...
        <svg class="ml-2 sm:ml-3 w-5 h-5 transition-transform duration-300" :class="active === {{ $i }} ? 'rotate-180' : ''" fill="none"
            stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
        </svg>
    </button>

    <div itemprop="acceptedAnswer" itemscope itemtype="https://schema.org" class="grid grid-rows-[0fr] transition-all duration-300 ease-in-out"
