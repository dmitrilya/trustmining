<div x-show="filter" class="fixed z-40" style="display: none" role="dialog" aria-modal="true">
    <div x-show="filter" class="fixed inset-0 bg-black bg-opacity-25" aria-hidden="true" x-transition:enter="transition-opacity ease-linear duration-300"
    ...
    <div x-show="filter"
        ...
        @click.away="filter = false" :style="{ top: document.getElementById('head')?.offsetHeight + 'px' }">
        <div class="flex items-center justify-between">
            <h3 class="text-lg text-slate-800 dark:text-slate-200">{{ __('Filters') }}</h3>

            <button type="button" class="flex h-10 w-10 items-center justify-center rounded-md bg-white dark:bg-slate-950 p-2 text-slate-500"
                @click="filter = false">
        ...
        <form class="mt-5" action="{{ route(request()->route()->getName(), request()->route()->originalParameters()) }}"
