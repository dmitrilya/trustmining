<div class="space-y-1 xs:space-y-2 sm:space-y-3">
    ...
    <a href="{{ route('metrics.coin.rate', ['coin' => 'bitcoin']) }}"
        class="text-sm font-bold block w-max under cursor-pointer {{ $attributes->get('active') == 'coin_rate' ? 'text-slate-700 dark:text-slate-200' : 'text-slate-500' }} hover:text-slate-700 dark:hover:text-slate-300">{{ __('Coin rate') }}</a>
</div>
