<nav x-data="{ open: false }" class="bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl border-b border-slate-100 dark:border-slate-800 sticky top-0 left-0 w-full z-50">
        ...
        <div class="flex justify-between h-10 lg:h-14">
            ...
            <div class="flex items-center ml-4 xl:ml-6">
                ...
                <div class="flex cursor-pointer mr-1 xs:mr-3">
                    <div
                        class="{{ app()->getLocale() == 'ru' ? 'bg-primary-gradient text-white' : 'bg-slate-100 hover:bg-slate-200 text-slate-800 dark:bg-slate-950 dark:hover:bg-slate-900 dark:text-slate-200' }} py-1 px-1.5 rounded-l-md border border-r-0 border-slate-300 dark:border-slate-700 text-xxs font-semibold">
                        <a href="{{ route('locale', ['locale' => 'ru']) }}">RU</a>
                    </div>
                    <div
                        class="{{ app()->getLocale() == 'en' ? 'bg-primary-gradient text-white' : 'bg-slate-100 hover:bg-slate-200 text-slate-800 dark:bg-slate-950 dark:hover:bg-slate-900 dark:text-slate-200' }} py-1 px-1.5 rounded-r-md border border-l-0 border-slate-300 dark:border-slate-700 text-xxs font-semibold">
                        ...
                        <x-dropdown align="right" width="48" contentClasses="bg-slate-100/95 dark:bg-slate-900/95 border border-slate-300 dark:border-slate-700">
                            ...
                            <x-slot name="content">
                                ...
                                @if (in_array($auth->role->name, ['admin', 'moderator', 'support']))
                                    ...
                                    <x-dropdown-link :href="route('roulette.prize.index')">
                                        {{ __('Roulette prizes') }}
                                    </x-dropdown-link>
                                @endif
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden lg:hidden">
        ...
        <div class="py-3 border-t border-slate-300 dark:border-slate-700">
            @auth
                <div class="px-4">
                    <div class="text-base text-slate-800 dark:text-slate-200">{{ $auth->name }}</div>
                ...
                <div class="mt-3 space-y-1">
                    ...
                    @if (in_array($auth->role->name, ['admin', 'moderator', 'support']))
                        ...
                        <x-responsive-nav-link :href="route('roulette.prize.index')">
                            {{ __('Roulette prizes') }}
                        </x-responsive-nav-link>
                    @endif
