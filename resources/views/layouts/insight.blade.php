            ...
            <div class="flex items-center">
                ...
                <div x-data="{ open: false }" @click.outside="open = false"
                    ...
                    <div @if (auth()->check() && auth()->user()->channel) @click="open = !open" @elseif (auth()->check()) @click="window.open('{{ route('insight.channel.create') }}', '_blank')" @else @click="$dispatch('open-modal', 'login')" @endif
                        class="group flex items-center cursor-pointer rounded-full bg-white/40 dark:bg-slate-900/40 border border-slate-300 dark:border-slate-700 shadow hover:shadow-lg shadow-logo-color hover:shadow-logo-color px-3 py-1.5 sm:px-4 sm:py-2 transition active:scale-95">
                    ...
                    @if (auth()->check() && auth()->user()->channel)
                        <div x-show="open" x-transition:enter="transition ease-out duration-300"
                            ...
                            class="absolute right-0 mt-2 w-48 origin-top-left rounded-xl border border-slate-300 dark:border-slate-700 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl shadow-lg z-50 overflow-hidden"
                            style="display: none;">
