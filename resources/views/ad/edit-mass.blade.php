<x-app-layout title="Обновление цен" noindex="true">
    ...
    <div class="max-w-7xl mx-auto px-2 py-4 sm:p-6 lg:p-8">
        <div class="bg-white/40 dark:bg-slate-900/40 border border-slate-300 dark:border-slate-700 shadow shadow-logo-color rounded-xl p-2 sm:p-4 md:p-6"
            ...
            <div class="divide-y divide-slate-300 dark:divide-slate-700">
                @foreach ($ads as $ad)
                    <div x-show="search === '' || '{{ $ad->asicVersion->asicModel->name . ' ' . $ad->asicVersion->hashrate . $ad->asicVersion->measurement }}'.toLowerCase().indexOf(search.toLowerCase()) !== -1"
                        ...
                        <div class="hidden xl:block text-slate-600 dark:text-slate-400 text-xxs sm:text-sm col-span-1">
