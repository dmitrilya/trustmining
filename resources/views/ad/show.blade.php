...
<x-app-layout :description="$description" :title="$title" :noindex="isset($moderation) ? 'true' : null" :canonical="$canonicalHref">
    ...
    <div class="max-w-8xl mx-auto px-2 py-4 sm:p-6 md:p-8">
                        ...
                        <div class="mx-auto md:grid md:grid-cols-12 md:grid-rows-[auto,auto,1fr] md:gap-x-8">
                            <div class="md:col-span-5">
                                <div class="h-full flex flex-col justify-between">
                                    <div class="flex gap-2{{ isset($moderation->data['preview']) || isset($moderation->data['images']) ? ' border border-indigo-500' : '' }}"
                                        x-data="{ active: 0 }">
                                        @php
                                            $modImages = isset($moderation->data['images']) ? $moderation->data['images'] : $ad->images;
                                            $modPreview = isset($moderation->data['preview']) ? $moderation->data['preview'] : $ad->preview;
                                        @endphp
                                        <div class="min-w-16 w-16 flex flex-col gap-2">
                                            <div @click="active = 0" class="w-full aspect-[4/3] rounded-lg cursor-pointer transition"
                                                :class="active === 0 ? 'ring-2 ring-indigo-500' : 'opacity-80 hover:opacity-100'">
                                                <img src="{{ Storage::url($modPreview) }}" alt="{{ $alt }} {{ __('Preview') }}"
                                                    class="w-full h-full rounded-lg object-cover">
                                            </div>

                                            @foreach ($modImages as $index => $image)
                                                <div @click="active = {{ $index + 1 }}" class="w-full aspect-[4/3] rounded-lg cursor-pointer transition"
                                        ...
                                        <div class="relative w-full aspect-[4/3] rounded-lg overflow-hidden bg-slate-100">
                                            <div x-show="active === 0" x-transition.opacity.duration.300ms class="absolute inset-0">
                                                <img itemprop="image" src="{{ Storage::url($modPreview) }}" alt="{{ $alt }}"
                                                    class="w-full h-full object-cover">
                                            </div>

                                            @foreach ($modImages as $index => $image)
                                                <div x-show="active === {{ $index + 1 }}" x-transition.opacity.duration.300ms class="absolute inset-0">
                                ...
                                <div class="md:col-span-2 md:col-start-1">
                                        ...
                                        @foreach ($ad->props as $prop => $value)
                                            @if ($prop != 'Service')
                                                @php
                                                    if ($prop == 'Modes') {
                                                        ...
                                                        $value = $modes->first()['h'] . $ad->asicVersion->measurement;
                                                        if ($modes->count() > 1) {
                                                            $value .= '-' . $modes->last()['h'] . $ad->asicVersion->measurement;
                                                        }
                                                    }
                                                @endphp
                                            ...
                                            @else
                                                <div class="text-xxs sm:text-xs md:text-sm text-slate-600 dark:text-slate-400">
                                                    <span>{{ __($prop) . ': ' }}</span><span
                                                        class="text-slate-800 dark:text-slate-200">{{ __($value) }}</span>
                                                </div>
                                            @endif
                                        @endforeach
                                    ...
                                    <div>
                                        @include('ad.components.about-seller', [
                                            ...
                                            'address' => [
                                                'city' => $ad->office->city,
                                                'street' => trim(implode(',', array_slice(explode(',', $ad->office->address), 2))),
                ...
                <div itemscope itemtype="https://schema.org/Product"
                        ...
                        <div class="mt-4 sm:mt-8 md:mt-0 md:col-span-7 md:border-l border-slate-300 dark:border-slate-700 md:pl-8">
                            <div class="flex items-start justify-between">
                                ...
                                <div
                                    ...
                                    <svg class="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 text-indigo-500" aria-hidden="true" width="24" height="24"
                            ...
                            <div itemprop="offers" itemscope itemtype="https://schema.org/Offer">
                                ...
                                @else
                                    <div itemprop="priceSpecification" itemscope itemtype="https://schema.org/PriceSpecification">
                                        ...
                                        <meta itemprop="priceCurrency" content="{{ $ad->coin->abbreviation != 'USDT' ? $ad->coin->abbreviation : 'USD' }}" />
                                        <meta itemprop="valueAddedTaxIncluded" content="{{ $ad->with_vat ? 'true' : 'false' }}" />
                                        <p itemprop="description" class="mt-5 text-2xl font-semibold text-slate-800 dark:text-slate-200 flex items-center">
                                            {{ __('Price on request') }}
                                        </p>
                                    </div>
                                @endif
                    ...
                    <div class="mt-8" x-data="{ selectedTab: 'description' }">
                        ...
                        <div x-show="selectedTab == 'description'">
                            <x-can-trust :user="$ad->user" :id="$ad->id" />
                        ...
                        <div x-show="selectedTab == 'location'" style="display: none">
                            @include('ad.components.location', ['location' => $ad->user->name . ', ' . $ad->office->address])
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex flex-col gap-4 w-full">
                        @if ($ad->adCategory->name == 'miners')
                            <div
                                ...
                                <script src="https://trustmining.ru/build/assets/calculator-widjet.js" data-theme="dark" data-blocks="additional-params,currency,coins"
                                    data-model="{{ $ad->asicVersion->asicModel->slug }}" data-version="{{ $ad->asicVersion->hashrate }}"></script>
                            </div>
                        @endif
