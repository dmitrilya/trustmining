    ...
    <div class="max-w-3xl mx-auto space-y-4" x-data="{ active: null }">
        <div
            ...
            <button @click="active !== 1 ? active = 1 : active = null"
                ...
                <span>Как осуществляется доставка {{ $ad->asicVersion->asicModel->name }}
            ...
            <div x-show="active === 1" x-collapse x-cloak>
                <div class="p-4 text-slate-600 dark:text-slate-400 border-t border-slate-200 dark:border-slate-800">
        ...
        <div
            ...
            <button @click="active !== 2 ? active = 2 : active = null"
                class="flex justify-between items-center w-full p-4 text-left font-semibold text-slate-800 dark:text-slate-200 transition-all">
                <span>Какая гарантия на {{ $ad->asicVersion->asicModel->asicBrand->name }}
        ...
        <div
            ...
            <button @click="active !== 3 ? active = 3 : active = null"
                class="flex justify-between items-center w-full p-4 text-left font-semibold text-slate-800 dark:text-slate-200 transition-all">
                <span>Как рассчитать окупаемость {{ $ad->asicVersion->asicModel->name }} на
        ...
        <div
            ...
            <button @click="active !== 4 ? active = 4 : active = null"
                class="flex justify-between items-center w-full p-4 text-left font-semibold text-slate-800 dark:text-slate-200 transition-all">
                <span>Можно ли доверять компании {{ $ad->user->name }}?</span>
                <svg class="ml-2 sm:ml-3 w-5 h-5 transition-transform duration-300"
        ...
        @if ($ad->user->hosting && $ad->user->tariff && $ad->user->tariff->can_have_hosting && !$ad->user->hosting->moderation)
            <div
                ...
                <button @click="active !== 5 ? active = 5 : active = null"
                    class="flex justify-between items-center w-full p-4 text-left font-semibold text-slate-800 dark:text-slate-200 transition-all">
                    <span>Могу ли я разместить приобретенное оборудование в майнинг-хостинге компании
                        {{ $ad->user->name }}?</span>
                    <svg class="ml-2 sm:ml-3 w-5 h-5 transition-transform duration-300"
