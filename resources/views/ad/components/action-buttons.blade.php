...
@else
    ...
    <div class="flex flex-wrap gap-3 sm:gap-4 mt-6">
        <a class="block w-full sm:w-max" target="_blank"
            ...
            <x-buttons.primary-button class="w-full flex items-center justify-center xs:py-3">
        ...
        @if (count($ad->user->phones))
            <x-buttons.secondary-button class="w-full sm:w-max justify-center bg-secondary-gradient dark:text-slate-800 xs:py-3"
        ...
        <x-buttons.secondary-button class="w-full sm:w-max justify-center xs:py-3" @click="{{ $trackClick }}">
            <svg class="min-w-4 h-4 mr-1 xs:mr-2" aria-hidden="true" width="24" height="24" fill="none"
                viewBox="0 0 24 24">
                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
