<div class="mt-2 sm:mt-3 grid grid-cols-1 {{ isset($cols2) ? $cols2 : 'xs:grid-cols-2' }} gap-2 sm:gap-4"
    x-data="{
        payback: '{{ $profit - $expense * $tariff > 0 ? $price / ($profit - $expense * $tariff) : '∞' }}',
        profit: {{ $profit }},
        expense: {{ $expense }},
        tariff: {{ $tariff }}
    }">
        ...
        <div class="relative z-10">
            ...
            <div class="mt-2 flex items-baseline gap-2">
                <span class="text-xl sm:text-3xl font-black"
                    :class="profit - expense * tariff > 1 ? 'text-emerald-600' : 'text-red-400'">
                    {{ number_format($profit, 2, '.', ' ') }}
                </span>
                <span class="font-bold"
                    :class="profit - expense * tariff > 1 ? 'text-emerald-600' : 'text-red-500/50'">USDT</span>
            </div>
        ...
        @if ($price != 0)
            <div class="mt-1 sm:mt-1.5 w-fit px-2 py-0.5 rounded text-xxs font-bold uppercase tracking-tighter"
                :class="{
                    ...
                    'bg-emerald-500/10 text-emerald-600': payback <= 730,
                }">
                <span
