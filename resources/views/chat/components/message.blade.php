...
@if (count($message->images))
    <div x-data="{ open: false }">
        <div class="flex {{ $auth->id == $message->user_id ? 'justify-end' : 'justify-start' }}">
            <div
                class="flex flex-col w-full max-w-[400px] leading-1.5 px-3 py-2 border-gray-200 border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 rounded-b-xl {{ $auth->id == $message->user_id ? 'ml-6 rounded-tl-xl' : 'mr-6 rounded-tr-xl' }}">
                <div class="flex items-center space-x-2 rtl:space-x-reverse mb-2">
                    <span class="date-transform text-xs font-normal text-gray-500 dark:text-gray-400"
                        data-date="{{ $message->created_at }}"></span>
                </div>
                        ...
                        <div class="group relative h-max max-h-60 max-w-max flex items-center overflow-hidden">
                            <div @click.self="$refs.image_preview.src = $el.nextElementSibling.src; open = true"
                                ...
                                <div id="download-image-{{ $message->id }}-{{ $loop->index }}" role="tooltip"
                                    class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-zinc-800">
                                    Download image
...
{{-- <button id="dropdownMenuIconButton" data-dropdown-toggle="dropdownDots" data-dropdown-placement="bottom-start" class="inline-flex self-center items-center p-2 text-sm font-medium text-center text-gray-900 bg-white rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none dark:text-white focus:ring-gray-50 dark:bg-zinc-950 dark:hover:bg-zinc-900 dark:focus:ring-zinc-700" type="button">
