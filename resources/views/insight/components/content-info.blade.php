<div class="flex items-center justify-between">
    ...
    <meta itemprop="{{ $type == 'video' ? 'uploadDate' : 'datePublished' }}" content="{{ $content->created_at->toIso8601String() }}" />
    ...
    <div class="ml-auto flex items-end">
        <div itemprop="interactionStatistic" itemscope itemtype="https://schema.org/InteractionCounter"
            ...
            <svg :class="{ 'block': liked, 'hidden': !liked }"
                @if ($user) @click="liked = false; likes--; window.like('{{ $type }}', {{ $content->id }})" @endif
            ...
            <svg :class="{ 'hidden': liked, 'block': !liked }"
                @if ($user) @click="liked = true; likes++; window.like('{{ $type }}', {{ $content->id }})" @endif
                class="w-4 h-4 text-gray-700 dark:text-white hover:text-gray-800 dark:hover:text-gray-200 cursor-pointer"
        ...
        <div itemprop="interactionStatistic" itemscope itemtype="https://schema.org/InteractionCounter"
            ...
            <p itemprop="userInteractionCount" class="text-xxs sm:text-xs text-gray-500 ml-2">
