<x-insight-layout :title="$article->title" :description="$article->subtitle" header="">
    @php
        $user = Auth::user();
    ...
    <div x-data="{ edit: false }"
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="ml-auto flex items-end">
                <div class="flex items-center" x-data="{ liked: '{{ $user && $article->likes->where('user_id', $user->id)->count() > 0 }}', likes: {{ $article->likes->count() }} }">
                    <svg :class="{ 'block': liked, 'hidden': !liked }"
                        @if ($user) @click="liked = false; likes--; window.like('article', {{ $article->id }})" @endif
                    ...
                    <svg :class="{ 'hidden': liked, 'block': !liked }"
                        @if ($user) @click="liked = true; likes++; window.like('article', {{ $article->id }})" @endif
                        class="w-4 h-4 text-gray-700 dark:text-white hover:text-gray-800 dark:hover:text-gray-200 cursor-pointer"
                ...
                <p class="text-xxs sm:text-xs text-gray-500 ml-2">{{ $article->views()->count() }}</p>
            </div>
        </div>
