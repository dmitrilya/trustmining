    ...
    <div x-data="{ edit: false }"
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="ml-auto flex items-end">
                <div class="flex items-center" x-data="{ liked: '{{ $user && $post->likes->where('user_id', $user->id)->count() > 0 }}', likes: {{ $post->likes->count() }} }">
                    <svg :class="{ 'block': liked, 'hidden': !liked }"
                        @if ($user) @click="liked = false; likes--; window.like('post', {{ $post->id }})" @endif
                    ...
                    <svg :class="{ 'hidden': liked, 'block': !liked }"
                        @if ($user) @click="liked = true; likes++; window.like('post', {{ $post->id }})" @endif
                        class="w-4 h-4 text-gray-700 dark:text-white hover:text-gray-800 dark:hover:text-gray-200 cursor-pointer"
