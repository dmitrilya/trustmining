<x-app-layout title="Поддержка, часто задаваемые вопросы"
    description="Центр поддержки пользователей: ответы на часто задаваемые вопросы по покупке, настройке и ремонту майнинг-оборудования. Инструкции по работе с личным кабинетом, оплате и доставке. Получите помощь экспертов и решите любой вопрос быстро">
    <div class="max-w-7xl mx-auto px-2 py-4 sm:p-6 lg:p-8" style="height: calc(100dvh - 104.4px)" x-data="{ tab: '{{ request()->tab ?? 'chat' }}' }">
        <div
            ...
            <ul class="flex text-sm text-center text-slate-600 dark:text-slate-200 rounded-t-2xl divide-x divide-slate-300 dark:divide-slate-700 border-b border-slate-300 dark:border-slate-700"
                ...
                <li class="w-full">
                    <button @click="tab = 'chat'" type="button" role="tab" aria-controls="chat" :class="{ 'text-indigo-500': tab == 'chat' }"
                ...
                <li class="w-full">
                    <button @click="tab = 'video'" type="button" role="tab" aria-controls="video" :class="{ 'text-indigo-500': tab == 'video' }"
            ...
            <div id="fullWidthTabContent" style="height: calc(100% - 52px)">
                ...
                <div x-show="tab == 'chat'"
                    class="h-full flex flex-col bg-white/40 dark:bg-slate-900/40 border border-slate-300 dark:border-slate-700 rounded-b-2xl p-1 sm:p-4"
                    ...
                    @else
                        <div class="bg-slate-100 dark:bg-slate-950 p-1 rounded-t-md h-full overflow-hidden">
                            <div class="bg-slate-100 dark:bg-slate-950 p-1 sm:p-5 h-full space-y-1 overflow-y-auto duration-100" id="chat-messages"
