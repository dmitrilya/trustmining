<x-app-layout title="Дорожная карта Trustmining — Планы развития и обновления проекта" description="Ознакомьтесь с официальной дорожной картой проекта Trustmining. Узнайте о планах развития платформы, запуске новых инструментов для майнинга, графике обновлений и стратегических целях нашей команды">
    <div class="lg:grid lg:grid-cols-5" style="height: calc(100vh - 64.4px)">
                    ...
                    <div class="w-full sm:max-w-md mt-6 px-6 py-4 overflow-hidden rounded-lg z-10">
                        <form method="POST" action="{{ route('register') }}">
                            ...
                            <div class="flex items-center justify-end mt-6 lg:mt-8">
                                <a class="underline text-sm text-slate-500 hover:text-white rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-slate-800"
                                    href="{{ route('login') }}">
                                    {{ __('Login') }}
                                </a>
        ...
        <div class="md:col-span-3 @auth lg:col-start-2 @endauth bg-white dark:bg-slate-950 relative overflow-y-auto">
            <svg class="absolute left-0 top-0 w-1.5 sm:w-2" aria-hidden="true" x-ref="timeline">
                <defs>
                    <pattern id=":S4:" width="6" height="8" patternUnits="userSpaceOnUse">
                        <path d="M0 0H6M0 8H6" class="stroke-slate-200 dark:stroke-slate-800" fill="none"></path>
                    </pattern>
                </defs>
            ...
            <div class="w-full px-4 sm:px-8 xl:px-12 py-8 space-y-12 sm:space-y-16 lg:space-y-20 xl:space-y-28 overflow-x-hidden"
                ...
                <article>
                    <header class="-ml-4 sm:-ml-8 xl:-ml-12 flex items-center mb-6 md:mb-8">
                        ...
                        <div class="date-transform text-xs text-slate-600 dark:text-slate-400 font-semibold" data-type="date"
                            data-date="{{ Carbon\Carbon::create('22-08-2025')->timestamp * 1000 }}"></div>
                    </header>

                    <div class="text-lg md:text-xl lg:text-2xl xl:text-3xl text-slate-800 dark:text-slate-200 font-semibold mb-2 lg:mb-4">
                    ...
                    <ul class="space-y-2 md:space-y-3 list-disc pl-8">
                        <li class="text-xs sm:text-sm text-slate-600 dark:text-slate-400 pl-2">{{ __('Invitation of large companies') }}
                        </li>
                        <li class="text-xs sm:text-sm text-slate-600 dark:text-slate-400 pl-2">{{ __('Testing basic functions') }}</li>
                        <li class="text-xs sm:text-sm text-slate-600 dark:text-slate-400 pl-2">{{ __('Bug fixes and optimization') }}</li>
                        <li class="text-xs sm:text-sm text-slate-600 dark:text-slate-400 pl-2">
