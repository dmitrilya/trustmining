<div class="mt-4 sm:mt-5 md:mt-6 lg:mt-7 grid grid-cols-4 sm:grid-cols-2 lg:grid-cols-4 gap-3">
    <a href="{{ route('company', ['user' => $user->slug]) }}"
        ...
        <div
            ...
            <svg class="h-5 sm:h-6 w-5 sm:w-6 {{ request()->routeIs('company') ? 'text-indigo-500' : 'text-slate-600 dark:text-slate-400 group-hover:text-indigo-600' }}"
    ...
    @php
        $hasCompany = $user->company && !$user->company->moderation;
        $hasHosting =
            $hasCompany &&
            $user->tariff &&
            $user->tariff->can_have_hosting &&
            $user->hosting &&
    ...
    <a href="{{ route('company.offices', ['user' => $user->slug]) }}"
        ...
        <div
            class="flex h-9 sm:h-11 w-9 sm:w-11 flex-none items-center justify-center rounded-lg bg-slate-50 dark:bg-slate-950">
            <svg class="h-5 sm:h-6 w-5 sm:w-6 {{ request()->routeIs('company.offices') ? 'text-indigo-500' : 'text-slate-600 dark:text-slate-400 group-hover:text-indigo-600' }}"
