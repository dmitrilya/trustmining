<div class="mt-4 sm:mt-5 md:mt-6 lg:mt-7 grid grid-cols-4 sm:grid-cols-2 lg:grid-cols-4 gap-3">
    <a href="{{ route('company', ['user' => $user->slug]) }}"
        ...
        <div
            ...
            <svg class="h-5 sm:h-6 w-5 sm:w-6 {{ request()->routeIs('company') ? 'text-indigo-500' : 'text-slate-600 dark:text-slate-400 group-hover:text-indigo-600' }}" aria-hidden="true"
    ...
    <a href="{{ $user->company ? route('company.about', ['user' => $user->slug]) : '#' }}"
        ...
        <div
            class="flex h-9 sm:h-11 w-9 sm:w-11 flex-none items-center justify-center rounded-lg bg-slate-50 dark:bg-slate-950">
            <svg class="h-5 sm:h-6 w-5 sm:w-6 {{ request()->routeIs('company.about') ? 'text-indigo-500' : 'text-slate-600 dark:text-slate-400 group-hover:text-indigo-600' }}" fill="none"
    ...
    <a href="{{ $user->hosting && !$user->hosting->moderation ? route('company.hosting', ['user' => $user->slug]) : '#' }}"
        ...
        <div
            class="flex h-9 sm:h-11 w-9 sm:w-11 flex-none items-center justify-center rounded-lg bg-slate-50 dark:bg-slate-950">
            <svg class="h-5 sm:h-6 w-5 sm:w-6 {{ $user->hosting && !$user->hosting->moderation ? request()->routeIs('company.hosting') ? 'text-indigo-500' : 'text-slate-600 dark:text-slate-400 group-hover:text-indigo-600' : 'text-slate-600 dark:text-slate-400' }}" fill="none"
                viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <a href="{{ route('company.offices', ['user' => $user->slug]) }}"
        ...
        <div
            class="flex h-9 sm:h-11 w-9 sm:w-11 flex-none items-center justify-center rounded-lg bg-slate-50 dark:bg-slate-950">
            <svg class="h-5 sm:h-6 w-5 sm:w-6 {{ request()->routeIs('company.offices') ? 'text-indigo-500' : 'text-slate-600 dark:text-slate-400 group-hover:text-indigo-600' }}" aria-hidden="true"
