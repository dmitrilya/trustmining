<div itemprop="author" itemscope itemtype="https://schema.org/Person"
    ...
    <div>
        <h4 itemprop="name"
            class="{{ isset($sm) ? 'mb-0.5 sm:mb-1 lg:text-sm' : 'mb-1 sm:mb-1.5 sm:text-sm lg:text-base' }} text-xs text-gray-700 dark:text-gray-300 font-bold">
            {{ $name }}</h4>
            ...
            <div class="relative">
                ...
                <div x-show="open" style="display: none"
                    ...
                    <div class="flex justify-center items-center">
                        ...
                        <div
                            class="ml-2 sm:ml-3 text-xs xs:text-sm sm:text-base text-gray-700 dark:text-gray-300">
                            {{ __($rank) }} ({{ $forumScore }})
                        </div>
                    </div>

                    <div class="flex items-center">
                        <p class="text-xxs xs:text-xs text-gray-500">{{ $prevRankScore }}</p>
                        <div class="mx-2 w-full bg-gray-200 rounded-full h-1.5 dark:bg-gray-700">
                            <div class="bg-indigo-600 h-1.5 rounded-full"
                                style="width: {{ (($forumScore - $prevRankScore) / ($rankScore - $prevRankScore)) * 100 }}%">
                            </div>
                        </div>
                        <p class="text-xxs xs:text-xs text-gray-500">{{ $rankScore }}</p>
                    </div>

                    <div class="space-y-0.5 sm:space-y-1">
                        @foreach ($ranks as $rankScore => $rank)
                            <p class="whitespace-nowrap text-xs xs:text-sm text-gray-700 dark:text-gray-300">{{ __($rank) }} <span class="text-gray-500">- {{ $rankScore }} {{ __('points') }}</span></p>
                        @endforeach
                    </div>
