<div itemprop="author" itemscope itemtype="https://schema.org/Person" class="mb-2 sm:mb-4{{ isset($sm) ? '' : ' lg:mb-6' }} flex items-center">
    <div
        class="{{ isset($sm) ? 'size-10 sm:size-12' : 'size-12 sm:size-14 lg:size-16 lg:mr-4' }} mr-2 sm:mr-3 rounded-full border border-indigo-500 p-0.5 overflow-hidden">
    ...
    <div>
        <h4 itemprop="name" class="{{ isset($sm) ? 'mb-0.5 sm:mb-1 lg:text-sm' : 'mb-1 sm:mb-1.5 sm:text-sm lg:text-base' }} text-xs text-gray-700 dark:text-gray-300 font-bold">
            {{ $name }}</h4>
        <div class="flex items-center{{ isset($sm) ? '' : ' xs:mb-0.5' }}">
            <svg class="{{ isset($sm) ? 'size-3 sm:size-4' : 'size-4 sm:size-5' }} text-gray-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
