<form action="{{ route('forum.answer.store') }}" method="POST" x-data="{ text: `{{ old('text') }}`, range: null, link_text: null, link_url: null }"
    ...
    <div class="px-4 py-2 bg-white dark:bg-slate-950 rounded-t-lg">
        ...
        <pre required id="text" aria-placeholder="{{ __('Your answer...') }}" x-ref="answer" contenteditable="true"
            class="cursor-text whitespace-normal resize-none w-full px-0 text-slate-950 dark:text-slate-200 bg-white border-0 dark:bg-slate-950 focus:ring-0 focus-visible:ring-0 focus-visible:outline-none dark:placeholder-slate-400"
    ...
    <div class="flex items-center justify-between px-3 py-2 border-t dark:border-slate-700" x-data="{ images: 0, files: 0 }">
        <div class="flex ps-0 space-x-1 rtl:space-x-reverse">
            <label for="input-file-answer"
                ...
                <input id="input-file-answer" name="files[]" class="hidden" type="file" accept=".pdf,.doc,.docx,.txt"
                    multiple
