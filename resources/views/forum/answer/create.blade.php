<form action="{{ route('forum.answer.store') }}" method="POST" x-data="{ text: `{{ old('text') }}` }"
    ...
    <div class="px-4 py-2 bg-white dark:bg-zinc-950 rounded-t-lg">
        <label for="text" class="sr-only">{{ __('Your answer...') }}</label>
        <textarea required id="text" rows="4" name="text" placeholder="{{ __('Your answer...') }}"
    ...
    <div class="flex items-center justify-between px-3 py-2 border-t dark:border-zinc-700" x-data="{ images: 0 }">
        <div class="flex ps-0 space-x-1 rtl:space-x-reverse">
            <label for="input-image-answer"
                ...
                <input id="input-image-answer" name="images[]" class="hidden" type="file" accept=".png,.jpg,.jpeg"
                    multiple
