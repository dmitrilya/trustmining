                ...
                <form action="{{ route('forum.question.store') }}" method="POST" x-data="{ theme: `{{ old('theme') }}`, text: `{{ old('text') }}`, range: null, link_text: null, link_url: null }"
                    ...
                    <div class="px-4 py-2 mt-6 bg-white dark:bg-slate-950 rounded-t-lg">
                        ...
                        <pre required id="text" aria-placeholder="{{ __('Your question...') }}" x-ref="question" contenteditable="true"
                            ...
                            @touchend="range = saveRange()" @paste="e => formatPaste($el, e)"></pre>
                    ...
                    <div class="flex items-center justify-between px-3 py-2 border-t dark:border-slate-700" x-data="{ images: 0, files: 0 }">
                        <div class="flex ps-0 space-x-1">
                            <label for="input-file-question"
                                ...
                                <input id="input-file-question" name="files[]" class="hidden" type="file" accept=".pdf,.doc,.docx,.txt" multiple
                            ...
                            <label for="input-image-question"
                                class="inline-flex justify-center items-center p-2 text-slate-600 rounded-md cursor-pointer hover:text-slate-800 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-slate-700">
                                <input id="input-image-question" name="images[]" class="hidden" type="file" accept=".png,.jpg,.jpeg,.webp" multiple
