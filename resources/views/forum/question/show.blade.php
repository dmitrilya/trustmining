<x-app-layout title="Форум по майнингу и криптовалюте TrustMining: {{ __($subcategory->name) }}"
    ...
    <div
        class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 py-8 grid grid-cols-1 lg:grid-cols-4 gap-3 sm:gap-5 xl:gap-7 items-start">
        <div
            class="bg-white dark:bg-zinc-900 overflow-hidden shadow-sm dark:shadow-zinc-800 rounded-lg p-4 md:p-6 col-span-3">
            <div class="flex justify-between">
                <h2
                    class="mb-1 sm:mb-3 lg:mb-5 text-sm xs:text-base sm:text-lg lg:text-xl text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-gray-100 font-bold">
                    {{ __($question->theme) }}
                </h2>

                <div class="text-right ml-3 sm:ml-5">
                    <div class="mb-0.5 sm:mb-1 text-xxs sm:text-xs lg:text-sm text-gray-500 whitespace-nowrap">
                        {{ __('Views') }}: <span>{{ $question->views_count }}</span>
                    </div>
                    <div class="text-xxs sm:text-xs lg:text-sm text-gray-500 whitespace-nowrap">
                        {{ __('Answers') }}: <span>{{ $question->forumAnswers->count() }}</span>
                    </div>
                </div>
            </div>

            <p class="text-xxs sm:text-xs lg:text-sm text-gray-500">{{ $question->text }}</p>
        </div>

        <div class="mt-4 sm:mt-6 lg:mt-0 space-y-4 sm:space-y-6">
            <div class="bg-white dark:bg-zinc-900 overflow-hidden shadow-sm dark:shadow-zinc-800 rounded-lg">
                <h2
                    class="mb-1 sm:mb-3 lg:mb-5 p-4 md:p-6 xs:text-lg sm:text-xl text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-gray-100 font-bold">
                    {{ __('Similar posts') }}
                </h2>
                ...
                <div class="divide-y divide-gray-100 dark:divide-zinc-800">
                    @foreach ($similarQuestions as $similarQuestion)
                        <a
                            ...
                            <div
                                class="px-4 py-2 xs:py-3 sm:px-6 sm:py-4 group hover:bg-gray-200 dark:hover:bg-zinc-950">
