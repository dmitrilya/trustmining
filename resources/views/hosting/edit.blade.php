            ...
            <form method="post" action="{{ route('hosting.update', ['hosting' => $hosting->id]) }}"
                ...
                <div class="relative mt-1" x-data="{ open: false, sugs: false }" @click.away="open = false">
                    ...
                    <ul role="listbox" style="display: none" x-show="open && sugs" x-ref="suggestionList"
                        class="absolute z-10 mt-1 w-full overflow-auto bg-white/40 dark:bg-slate-900/40 border border-slate-300 dark:border-slate-700 backdrop-blur-xl rounded-lg shadow-lg shadow-logo-color focus:outline-none">
                    </ul>
                </div>
