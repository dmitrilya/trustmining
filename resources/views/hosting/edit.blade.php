    ...
    <div class="max-w-3xl mx-auto px-2 py-4 sm:p-6 lg:p-8">
        <div
            ...
            <form method="post" action="{{ route('hosting.update', ['hosting' => $hosting->id]) }}"
                ...
                quill.root.innerHTML = `{{ $hosting->description }}`;
                ...
                <div class="relative mt-1" x-data="{ open: false, sugs: false }" @click.away="open = false">
                    <div class="relative z-0 w-full group" @click="open = true">
                        ...
                        <label for="address"
                            class="absolute text-sm text-slate-600 dark:text-slate-300 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-indigo-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                ...
                <div>
                    ...
                    <x-text-input id="video" name="video" type="text" :value="$hosting->video" autocomplete="video" />
                ...
                <x-editable-list name="conditions" :items="$hosting->conditions">
                    <x-input-label :value="__('Conditions (from N units, from N th, etc.)')" />
                </x-editable-list>

                <x-editable-list name="expenses" :items="$hosting->expenses">
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500" id="contract_help">DOC (max. 1MB)</p>
                ...
                <div>
                    ...
                    <x-file-input id="hosting-territory" name="territory" class="mt-1 block w-full"
                        accept=".doc,.docx" />
                    <p class="mt-1 text-sm text-slate-500" id="territory_help">DOC (max. 1MB)</p>
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500" id="energy_supply_help">DOC (max. 1MB)</p>
                ...
                <div class="mt-6 flex justify-end">
                    <x-danger-button x-data="" type="button"
                        @click.prevent="$dispatch('open-modal', 'confirm-hosting-deletion')">{{ __('Delete a hosting') }}</x-danger-button>
            ...
            <x-modal name="confirm-hosting-deletion">
                <form method="post" action="{{ route('hosting.destroy', ['hosting' => $hosting->id]) }}"
                    ...
                    <h2 class="text-lg text-slate-950 dark:text-slate-50">
                        {{ __('Are you sure you want to delete this hosting?') }}
                    </h2>
