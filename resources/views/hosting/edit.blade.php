    ...
    <div class="max-w-3xl mx-auto px-2 py-4 sm:p-6 lg:p-8">
        <div
            ...
            <form method="post" action="{{ route('hosting.update', ['hosting' => $hosting->id]) }}"
                ...
                <x-inputs.editable-list name="conditions" :items="$hosting->conditions">
                    <x-inputs.input-label :value="__('Conditions (from N units, from N th, etc.)')" />
                </x-inputs.editable-list>

                <x-inputs.editable-list name="expenses" :items="$hosting->expenses">
                ...
                <div class="mt-6 flex justify-end">
                    <x-buttons.danger-button x-data="" type="button"
                        @click.prevent="$dispatch('open-modal', 'confirm-hosting-deletion')">{{ __('Delete a hosting') }}</x-buttons.danger-button>
