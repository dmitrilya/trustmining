<x-app-layout :title="__('meta.tariff.show.title', ['name' => $tariff->name])" :description="__('meta.tariff.show.description', ['name' => $tariff->name])">
    <div class="bg-slate-900 h-128 lg:h-144 xl:h-160 relative z-10 overflow-hidden">
        <div class="max-w-6xl mx-auto px-2 py-4 sm:p-6 lg:p-8 relative">
            ...
            <svg viewBox="0 0 1208 1024" class="absolute -translate-x-1/2 left-1/2 top-1/2 h-[60rem] -z-10"
                ...
                <ellipse cx="604" cy="512" rx="604" ry="512" fill="url(#6d1bd035-0dd1-437e-93fa-59d316231eb0)"></ellipse>
                <defs>
    ...
    <div class="bg-slate-100 relative pb-16 z-20">
        <div class="max-w-6xl mx-auto px-2 sm:px-6 lg:px-8 pt-8 pb-16 relative">
            <div
                ...
                <div
                    class="{{ $tariff->name == 'Subscription' || $tariff->name == 'Enterprise' ? 'text-slate-200' : 'text-slate-800' }} font-semibold text-lg md:text-2xl">
                    {{ $tariff->name }}</div>
                <div class="{{ $tariff->name == 'Subscription' || $tariff->name == 'Enterprise' ? 'text-slate-400' : 'text-slate-600' }} text-sm md:text-base">
                    {{ __($tariff->description) }}</div>
                <div
                    class="h-9 flex items-end {{ $tariff->name == 'Subscription' || $tariff->name == 'Enterprise' ? 'text-slate-400' : 'text-slate-600' }} md:text-lg">
                    <span
        ...
        <div class="max-w-4xl mx-auto bg-white overflow-hidden shadow rounded-xl p-4 sm:p-8 md:p-12 mt-8">
            ...
            <form method="post" action="{{ route('order.store') }}" class="space-y-4 sm:space-y-6">
                ...
                <x-inputs.text-input name="amount" type="hidden" min="100" :value="$tariff->price * 30" autocomplete="off" required />

                <div class="grid grid-cols-3 gap-1 xs:gap-2 sm:gap-4 md:gap-6">
                    <div>
                        <input class="sr-only peer" type="radio" name="method" id="qr" value="qr" required checked>
                            ...
                            <svg class="h-4 xs:h-6 sm:h-8" viewBox="0 0 178 86" fill="none">
                                <g clip-path="url(#clip0_3_497)">
                                    <path
                                        ...
                                        fill="#1B0A40" />
                                    <path d="M91.7947 57.1272V51.6064H92.7792V55.7153L95.9708 51.6064H96.9394V57.1272H95.9708V53.0184L92.7792 57.1272H91.7947Z"
                                        fill="#1B0A40" />
                                    <path
                ...
                <x-buttons.primary-button class="block w-full sm:w-max ml-auto md:!mt-10">{{ __('Pay') }}</x-buttons.primary-button>
            </form>
        </div>
    </div>
</x-app-layout>
