<div
    class="bg-white/40 dark:bg-slate-900/40 border border-slate-300 dark:border-slate-700 shadow shadow-logo-color rounded-xl p-2 sm:p-4 lg:p-6 space-y-4 lg:space-y-6">
    @php
        ...
        $versionWithAds = $modelAds->where('price', '!=', 0)->count()
            ? $model->data->asicVersions
                ->where(
                    'hashrate',
                    (float) $ads->where('asic_model_slug', $model->slug)->where('price', '!=', 0)->sortByDesc('asic_version_hashrate')->first()->asic_version_hashrate,
                )
                ->first()
            : null;
        if ($versionWithAds) {
