<div x-data="{
...
}" x-init="setTimeout(() => axios.get('/calculator/get-models').then(r => models = r.data), 10000);">
    ...
    <template x-for="asicModel in models" :key="asicModel.id">
        <div x-show="selectedModel == asicModel.id" class="mt-6" x-data="{ open: false }">
            <x-input-label :value="__('Version')" />

            <div class="relative mt-1" @click.away="open = false">
                <button type="button" @click="open = !open" :id="'dropdown-version_' + asicModel.id + '_button'"
                    ...
                    <span class="flex items-center">
                        <span class="ml-3 block truncate"
                            x-text="selectedVersion == null ? '' : models.find(function(model) {return model.id == selectedModel}).asic_versions.find(function(version) {return version.id == selectedVersion}).hashrate"></span>
                    </span>
                ...
                <ul class="absolute z-10 mt-1 max-h-56 w-full overflow-auto rounded-md bg-white dark:bg-zinc-900 py-1 text-base shadow-lg dark:shadow-zinc-800 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm"
                    ...
                    <template x-for="asicVersion in asicModel.asic_versions" :key="asicVersion.id">
                        <li @click="selectedVersion = asicVersion.id; open = false; version = asicVersion"
                            role="option"
                            class="relative cursor-default select-none py-2 pl-3 pr-9 text-gray-950 dark:text-gray-50 hover:bg-indigo-600 hover:text-white">
                            <div class="flex items-center">
